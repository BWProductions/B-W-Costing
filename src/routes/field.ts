...
function registerFleetRoutes(fleetApp: Hono<Env>, cfg: FleetConfig) {
  ...
  fleetApp.get('/success/:id', async (c) => {
    ...
    // Bibi's spec: driver's name, what vehicle + reg, the date. Same shape as
    // the B&W flow but tailored for music bus context. We look up the short
    // vehicle name (e.g. "Iveco 50C15V15 PV (Mthatha bus)") so the recipient
    // immediately knows which bus this is, not just a reg plate.
    const shortName = await getVehicleShortName(c.env.DB, cfg.formType, sub.vehicle_reg || '')
    const cleanReg = (sub.vehicle_reg || '').replace(/\s+/g, '').toUpperCase()
    const pdfUrl = sub.pdf_url || `https://bwprodsystem.co.za/field/pdf/${sub.id}`
    const pdfDownUrl = pdfUrl + (pdfUrl.includes('?') ? '&' : '?') + 'download=1'
    const niceDate = sub.delivery_date ? formatDate(sub.delivery_date) : ''
    const waMsg = encodeURIComponent(
      `🎵 *${cfg.brandTitle} Inspection* — Ref: ${sub.form_number}\n` +
      `👤 Driver: ${sub.prepared_by || '—'}\n` +
      `🚐 Vehicle: ${shortName ? shortName + ' — ' : ''}${cleanReg || sub.vehicle_reg || '—'}\n` +
      `📅 Date: ${niceDate}\n` +
      `✅ Passed: ${passes} / ${INSPECTION_ITEMS.length}${fails > 0 ? `   ❌ Failed: ${fails}` : ''}\n` +
      `\n📄 Tap to view PDF: ${pdfUrl}\n` +
      `⬇️ Tap to download: ${pdfDownUrl}`
    )
    ...
    <div class="field-wrap" style="max-width:560px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px;padding:14px 18px 24px">
        <!-- WhatsApp share — the prominent action. Opens WhatsApp with the
             driver + vehicle + reg + date message pre-typed; recipient sees
             the PDF link and can save/forward. -->
        <a href="https://wa.me/?text=${waMsg}" target="_blank" rel="noopener"
           style="display:flex;align-items:center;justify-content:center;gap:10px;padding:14px 24px;border-radius:10px;background:#25D366;color:#fff;font-weight:800;text-decoration:none;font-size:15px;box-shadow:0 4px 14px rgba(37,211,102,0.3)">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
          Send to WhatsApp
        </a>
        <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
          <a href="/field/pdf/${sub.id}" target="_blank" class="btn-submit no-print" style="background:${cfg.accentColor};color:#000;font-weight:800;text-decoration:none;display:inline-flex;align-items:center;gap:8px;padding:12px 24px;flex:1;justify-content:center;min-width:140px">
            📄 View PDF
          </a>
          <a href="/field/pdf/${sub.id}?download=1" class="btn-submit no-print" style="background:rgba(34,211,238,0.12);border:1px solid rgba(34,211,238,0.4);color:${cfg.accentColor};font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:8px;padding:12px 24px;flex:1;justify-content:center;min-width:140px">
            ⬇️ Download
          </a>
        </div>
        <a href="${cfg.basePath}" class="btn-submit no-print" style="background:rgba(255,255,255,0.05);color:#fff;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:12px 24px;border:1px solid var(--border)">
          ← New inspection
        </a>
      </div>
    </div>`

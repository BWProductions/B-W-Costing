...
export async function buildPayrollWorkbook(deps: PayrollDeps): Promise<{ bytes: Uint8Array, filename: string, checks: Record<string, number> }> {
  ...
  const revRes = staffIds.length ? await db.prepare(`SELECT id, issue_key, status, severity, staff_id, staff_name, work_date, subject_source, subject_shift_id, compared_source, compared_shift_id, compared_payroll_week_start,
        ...
        decision_type, decision_reason, reviewed_by_name, reviewed_at, system_snapshot_json, compared_snapshot_json, subject_snapshot_json, warning_kind, payroll_week_start
  ...
  // Overlap reviews: full both-sides wording (owner 2026-09-16). Display only.
  let liveEntries: Record<string, LiveEntry> = {}
  try { liveEntries = await loadLiveEntries(db, reviewsAll as any) } catch (e) { liveEntries = {} }
  const flagText = (sn: any, v: ReviewRow): string => {
  ...
  const priced: Priced[] = paid.map((r) => {
    ...
    const sysParts = allReviews.map((v) => { const sn = parseSnap(v.system_snapshot_json); const rec = v.system_proposed_payable_hours !== null && v.system_proposed_payable_hours !== undefined ? ` System recommended ${Number(v.system_proposed_payable_hours).toFixed(2)} h${sn?.recommendedStart ? ' (' + sn.recommendedStart + '–' + sn.recommendedEnd + ')' : ''}.` : ''; return `#${v.id} ${flagTitle(sn, v)}: ${flagText(sn, v)}${rec}` })

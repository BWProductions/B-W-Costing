...
## Restore point 22 Sep 2026 — `restore-2026-09-22-petrus-v5` (live v2026-09-22-14)
...
- Crew pattern & venue-name check (`src/crew-pattern.ts`, v2026-09-22-9): RED when a venue-rate entry has no venue name; ORANGE when ≤25% of a day's crew (≥4 workers) say a different place than the rest. Paid rows AND drafts decided with two place buttons WAREHOUSE R81,25/h / VENUE R95/h (each shows the rand); paid rows re-price now, draft decisions (snapshot.placeDecision) are applied in applyOwnerRatesToFinalSubmission. Current unpaid week only.
- Overlap reviews render a "What overlaps what" box (already billed vs this entry, exact overlap hours, hours not covered, recommendation). Draft-vs-own-paid-row reviews auto-voided on dashboard load (v2026-09-22-10).
- Public holidays (rate rules v10, `SA_PUBLIC_HOLIDAYS`): every holiday entry held as red review `public_holiday|shift:ID` (approve / other amount / decline via `/wages-admin/petrus-extra`). Recommended: Warehouse R162,50/h (meeting deducted first Mon–Fri), Venue R190/h, Petrus R160/h no fixed day, Music Bus R750 07–16 + R180/h outside. Election Day 4 Nov 2026 included.
...
## Restore point 23 Sep 2026 (3) — `restore-2026-09-23-payroll-final` (live v2026-09-23-13) — payroll 19–25 Sep sent to auditors
- **Owner sentence + single APPROVE** on already-paid place reviews (v-3): "Paid last week … / 06:00–18:00 at a venue … / Less the Garden already paid / WHAT YOU SHOULD PAY THIS WEEK"; Venue default when Area names a venue. Givemore #10073 → R640.
- **Warehouse Team cross-check** (v-4, `crew-pattern.ts` `wordingNamesAVenue` / `venueEvidence`): venue, area AND description checked; flag `WAREHOUSE CLAIMED BUT THE ENTRY NAMES "…"`. Rule: Warehouse Team = warehouse only if the place says warehouse; FNB/Riverside = venue; House = garden (Takavaudza Sun House 6 h = R375).
- **Holiday rule**: 07:00–07:30 meeting deducted on public holidays too, then ×2 (07–15 warehouse = 7,5 h × R162,50 = R1 218,75). Gardener holiday R125/h. Heritage Day 24 Sep set for everyone 07:00–15:00 warehouse (Patrick 06:30 R1 300; Petrus 06:30–14:00 R1 120; Givemore 08–15 R875; John 07–12 warehouse + 12–18 Music Bus R1 110).
- **Planned end time** (v-7): table `wage_planned_hours (work_date PK, planned_end, note, set_by)`; 2026-09-24 → 15:00; red `⏰ CLAIMED PAST hh:mm` pill (`beyondPlanned`).
- **Public-holiday follow-up panel** `#bw-holiday-followup` (v-11): the week after a planned holiday, per-row real-end-time drop-down with rand, `POST /wages-admin/confirm-holiday-hours` inserts a ± correction row in the current payroll and marks the row `HOLIDAY HOURS CONFIRMED`.
- **Hours column** shows paid vs clocked hours (v-5). **Green figure** uses owner-corrected amounts; R0 rows count 0 h (v-12).
- **Double-tap protection** (v-11): client `form.__bwSubmitting` lock + `#bw-saving-overlay` ("Saving… please wait", 20 s unlock); server refuses a NEW draft with same date/start/end as an existing draft or paid row (`/wages/drafts` POST, redirect to `/wages/me?error=…`); Final Submission refused when the hours are already covered by a paid row (v-5). All refusals logged to `wage_debug_capture`.
- **Pay-rule check UI retired** (owner: only that) — `personRuleHtml = ''`, `ruleSummaryPill = ''`, tile removed. All other flags unchanged.
- **Excel** (`payroll-excel.ts`): `manager_update_reason` rows are final ("OWNER CORRECTION: …"); DIFFERENCE ONLY rows; paid-before R0 not applied to rows the owner set > 0; `decidedFor` excludes crew_pattern. Verified 0 mismatches Excel vs DB → `docs/BW_Payroll_2026-09-19_to_25_AUDITOR.xlsx` (91 rows R61 933,86 + Sharleen R3 000 = R64 933,86, 0 open reviews).
- Staff decisions 23 Sep (all in handover Part 6b): John Wed 16 R105 / Thu 17 R480 / Mon 21 dup R0 → R6 461,26; Joshua Pastdal Sat 13 R0 + Wed 23 dup R0; Erick FNB row re-dated to Thu 17 16–18 R190 + Mon 21 warehouse R690,63; Solomon Tue warehouse R690,63; Lebo / Thandanani drafts deleted.
- **Loans**: #7 Erick R500 (22 Sep, R500 from 26 Sep), #8 Isaac R500 (22 Sep, R250 × 2 from 26 Sep) — `deduction_start_date = 2026-09-26`, `created_by_user_id = 3`; #5 Daniel marked `paid` (settled outside payroll); #6 Isaac R600 deducted this week as scheduled.
- Backups: `docs/restore-2026-09-23-{givemore,john,john2,joshua,holiday,final,loans}/`. Handover: `docs/handover-2026-09-22-complete-rules.md` / `.pdf` (updated 23 Sep).
- **v-13**: `/wages-admin/confirm-holiday-hours` creates a top-up row only when `diff > 0` (owner: "only if we underpaid; if they worked less, nothing"); worked-less is recorded in `payroll_note` only. Drop-down 11:00–18:00 half-hourly with UNDERPAID/worked-less labels. Petrus holiday pricing (`ownerPayForShift` kind petrus) deducts the 07:00–07:30 meeting Mon–Fri.
